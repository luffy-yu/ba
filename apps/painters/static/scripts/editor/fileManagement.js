...
function validateFileName() {
    ...
    var valid = true;
    if (!name.match(/^[\w,\s-]{1,255}.(proj|sess|yaml)$/)) valid = false;
...
function update_fileData() {
    $('.squareButton.file').each(function (index, value) {
        // TODO
        fileData[value.textContent] = [];
    });
}
...
document.getElementById("createFileName").addEventListener("click", function () {
    ...
    $.ajax({
        ...
        error: function (data) {
            console.error(data.status, data.responseText);
            alert('Failed! Details were in the console.');
        }
    });
});

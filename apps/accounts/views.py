...
class ProfileView(UnbannedUserMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context['user'] = ElementalUser.objects.get(username__iexact=self.kwargs.get('username', self.request.user.username))
        if not self.request.user.is_anonymous and not self.request.user.is_superuser and len(
                self.request.user.groups.all()[0].name) > 0:
            auth_group = self.request.user.groups.all()[0].name

        # just a little failsafe in case of broken things...
        allowed_groups = ('admin', 'moderator',)
        if context['user'].trusted or self.request.user.is_superuser or auth_group in allowed_groups:
